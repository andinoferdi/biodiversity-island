...
- Langkah kedua fase Hari 6–9 (animal species) `Implemented` di branch `andino/feat/biome-terrain` — enam spesies GLB dari `public/assets/animal/` menggantikan 3 placeholder, roster 12 individu di bioma rumah masing-masing, deer beranimasi (`SkeletonUtils.clone` + `useAnimations`, klip mengikuti status simulasi dan kontrol waktu), model statis via drei `Clone`, Suspense per hewan (`src/components/prototype/AnimalModel.tsx`, `species.ts`, `Animal.tsx`); lint serta production build lulus (dokumentasi: `docs/feature/animal-species/animal-species.md`)

Fase Hari 3–5 (Simulasi Inti) selesai; fase Hari 6–9 (Bioma & Spesies) `In Progress` — biome terrain dan animal species selesai. Fitur lanjutan fase ini (predator–mangsa, krisis) dan fase berikutnya (save lokal, dll.) belum ada di source code. Dependency dan fitur harus selalu diverifikasi dari `package.json` dan source code sebelum dianggap sudah terpasang atau selesai.
...
3. **Hari 6–9 — Bioma & Spesies** (`In Progress` — biome terrain dan animal species `Implemented`: tiga bioma, sumber daya per bioma, enam spesies GLB dengan deer beranimasi; predator–mangsa dan krisis belum ada): tiga bioma, enam spesies GLB, animasi, predator–mangsa, serta tiga krisis.
