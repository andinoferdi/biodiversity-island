...
- `In Progress` (Hari 6–9, lanjutan): predator–mangsa — kill reward via mailbox `killRewards`, sense pass per frame (mangsa tak lagi kebal saat makan/minum), wolf sebagai predator darat baru dengan klip Gallop, baris "Prey" di panel (`src/components/prototype/simulation.ts`, `Animal.tsx`, `species.ts`, `AnimalModel.tsx`, `BiodiversityPrototype.tsx`) — lint & build lulus, dikalibrasi visual di browser. Bug kolaps populasi umum pre-existing **fixed** (lihat `docs/feature/remaster-wahyu/remaster-wahyu.md`); **balancing masih belum tuntas**: sensus 10 menit @4× stabil di cap 24 tanpa kepunahan total, tetapi berakhir monokultur hawk — deer/rabbit/duck/fish habis dalam ±2 menit-simulasi (data trajektori di dokumen remaster). Tiga krisis ekosistem belum ada
